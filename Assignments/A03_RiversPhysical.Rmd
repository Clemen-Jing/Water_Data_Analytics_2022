...
> The highest flows occur in the winter. We don’t see a strong snowmelt pulse in the spring, so the
climate is likely warm enough that snow is uncommon. This watershed likely experiences higher
...
> The 30-year recurrence interval calculation results in a greater rise in discharge for each unit
increase in recurrence interval than for the full calculation. A 100-year flood computed from a
30-year record is predicted at a higher discharge (5516 cfs) than that predicted from the full record
(4835 cfs). These numbers are in the same magnitude and differ only ~10 % from each other, so
the system is not displaying strong trends over time. We might assume this system is displaying
stationarity [another acceptable response would be that flooding is decreasing slightly overall].
