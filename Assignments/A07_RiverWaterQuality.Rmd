...
```{r}
NewHopeWQ <- readWQPqw(siteNumbers = "USGS-02097314",
                       parameterCd = c("00300"))
...
> Hypoxia only occurred in the 1980s and remained high after that.
...
DOplot <-
  ...
  geom_point()
TPplot <-
  ggplot(NewHopeTP, aes(x = Date, y = TP_mgL)) +
  geom_point()

plot_grid(DOplot, TPplot, ncol = 1)
```
...
> Higher nutrients when there is more hypoxia. Probably related.
...
```{r}
DOplot <-
  ggplot(subset(NewHopeDO, Date < "1985-01-01"),
         aes(x = Date, y = DO_mgL)) +
  geom_point()

dischargeplot <-
  ggplot(subset(NewHopeDischarge, Date < "1985-01-01"),
         aes(x = Date, y = Discharge_cfs)) +
  geom_line()

plot_grid(DOplot, dischargeplot, ncol = 1)
```

10. Do hypoxic events happen in the wet or the dry season? Why might that be?

> Dry season. Longer water residence time during dry periods, meaning the water isn’t flushed out as fast and DO has a chance to be drawn down.
