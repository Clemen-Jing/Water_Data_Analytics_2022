...
# as.Date() is to change the data as a date. 
class(NTLdata$sampledate)
...
# NTLdata <- drop_na(NTLdata, temperature_C)  has the same function
dim(NTLdata)  
...
#temperature decrease as the depth, and the tempratrue varies a lot in the surface. Maybe in winter the river would freeze.
...
# in winter, the surface freezes, and the hypolimnion's temprature is still above 0. 4 degree is the temperature where water has the largest density.
...
# dimictic "s, |, reversed s, |" where "|" means the water mix well, as it may freeze, "s" or "reversed s" means that the water stratify into layers with different temperature. 
# monomictic "|, |, reversed s, |"
# polymictic they are too shallow to develop thermal stratification; thus, their waters can mix from top to bottom throughout the ice-free period. 
# amictic: perennially sealed off by ice, from most of the annual seasonal variations in temperature.
...
# epilinion has higher dendity of DO as atmopsphere, algea, plants, 
# at the layer with highest density of DO, there are the most nutrients and the plants here can also access to the sunset, so they florish here. 
```
