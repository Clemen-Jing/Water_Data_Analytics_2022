...
> soil satuarated, veges may take up the water or maintain the water, impervious surfaces
...
library(EcoHydRology) #load this first as it may include some other ones.
library(tidyverse)
...
#?baseflow
# In this package, the first 12 data and the last 12 data would be NA.
...
t.test(EnoDischarge$Baseflow_lfstat, EnoDischarge$bt, paired = T)
...
EnoDischarge_summary <- EnoDischarge %>%
  group_by(WaterYear) %>%
  summarise(Discharge.acft.yr = sum(Discharge)*723.968, # what is this factor? cfs -> acrefeet per year
...
```{r}
DCDischarge <- readNWISdv(siteNumbers = "07139500",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "1960-01-01", 
                     endDate = "2005-12-31")

names(DCDischarge)[4:5] <- c("Discharge", "Approval.Code")

ggplot(DCDischarge, aes(x = Date, y = Discharge)) +
  geom_line() +
  labs(x = "Year", y = "Discharge (cfs)")

DCDischarge <- DCDischarge %>%
  mutate(Baseflow_lfstat = baseflow(Discharge), 
         Stormflow_lfstat = Discharge - Baseflow_lfstat,
         Year = year(Date), 
         WaterYear = water_year(Date))
...
DCDischarge_summary <- DCDischarge %>%
  group_by(WaterYear) %>%
...
DCDischarge_summary$WaterYear <- as.numeric(as.character(DCDischarge_summary$WaterYear))


ggplot(DCDischarge_summary, aes(x = WaterYear, y = Discharge.acft.yr)) +
  geom_line()+
  geom_line(aes(y = Baseflow.acft.yr), lty = 2)

ggplot(DCDischarge_summary, aes(x = WaterYear, y = Prop.bf)) +
  geom_col()
```
