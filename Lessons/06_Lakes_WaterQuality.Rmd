...
ggplot(cars, aes(x = speed))+
  geom_line(aes(y = dist, color = LakeHuron)) +
  scale_linetype(values = c("grey", "red"))
```
...
nutrients(N,P), sediments, heavy metal, bacteria, fungal
...
**0-40**: Oligotrophic: low nutrients
**40-50**: Mesotrophic: medium nutrients
**50-70**: Eutrophic: rich nutrients
**70-100**: Hypereutrophic: high nutrients
...
ggplot(LAGOStrophic, aes(x = as.factor(samplemonth), y = chla)) +
  geom_jitter(alpha = 0.2, size = 0.5) + #tells the actual sample numbers in the data set
...
#violin plot: like boxplot, but also show the median concentration 
#40, 50, 70: the boundary of Oligotrophic, Mesotrophic, Eutrophic, Hypereutrophic.

ggplot(LAGOStrophic, aes(x = state, fill = trophic.class)) +
  geom_bar(stat = "count", position = position_dodge(preserve = "single")) +
  #preserve = "single") keep the position for the bar tat is 0
  theme(legend.position = "top") +
