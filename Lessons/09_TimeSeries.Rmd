...
> Seasonal trends, the increase in the peak discharge, whether it's a obvious or random trend. and the peak events happened every twenty years. 
...
#store as time-series, values, not data
```
...
#trend is the average of 30-day values
...
NeuseCond_processed <- NeuseCond %>%
  ...
  arrange(Date)
#?floor_date:floor_date() takes a date-time object and rounds it down to the nearest boundary of the specified time unit.

NeuseCond_monthly <- left_join(Months, NeuseCond_processed)
#?left_join:If a row in x matches multiple rows in y, all the rows in y will be returned once for each matching row in x.
...
```{r}
NeuseCond_monthly_ts <- ts(NeuseCond_monthly[[3]], frequency = 12)
#frequency means to divide a year in to how many pieces.
NeuseCond_monthly_Decomposed <- stl(NeuseCond_monthly_ts, s.window = "periodic")

# Visualize the decomposed series. 
plot(NeuseCond_monthly_Decomposed)

NeuseCond_monthly_Components <- as.data.frame(NeuseCond_monthly_Decomposed$time.series[,2:4])
NeuseCond_monthly_Components <- mutate(NeuseCond_monthly_Components,
                      Observed = NeuseCond_monthly$Conductivity,     
                      Date = NeuseCond_monthly$Date)

# Visualize how the trend maps onto the data
ggplot(NeuseCond_monthly_Components) +
  geom_line(aes(y = Observed, x = Date), color = "gray", size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  geom_hline(yintercept = 0, lty = 2) +
  labs(x = "", y = "Discharge (cfs)")
